...
**/
    ...
    if (*RootPtr != NULL) { //mame neprazdny strom
        do {
            where = helper;
            if (helper->Cont > Content) { //lava cast stromu
                helper = helper->LPtr;
            } else if (helper->Cont < Content) { //prava cast stromu
                helper = helper->RPtr;
            } else {
                found = TRUE;
            }
    ...
    if (found == TRUE) { //prvok, ktory chceme vlozit uz existuje, tak ho nahradime
        where->Cont = Content;
    } else {
        if (where == NULL) { //prazdny strom
            tBTNodePtr new = malloc(sizeof(struct tBTNode));
            new->RPtr = NULL;
            new->LPtr = NULL;
            new->Cont = Content;
            *RootPtr = new;
        } else if (where->Cont > Content) { //novy uzol stromu ide dolava
            tBTNodePtr new = malloc(sizeof(struct tBTNode));
            new->RPtr = NULL;
            new->LPtr = NULL;
            new->Cont = Content;
            where->LPtr = new;
        } else { //novy uzol stromu ide doprava
            tBTNodePtr new = malloc(sizeof(struct tBTNode));
            new->RPtr = NULL;
            new->LPtr = NULL;
            new->Cont = Content;
...
**/
    if (RootPtr != NULL) //prazdny strom
    {
        ...
        tBTNodePtr helper = RootPtr;
        while (!SEmptyP(&stack)) {
            helper = STopPopP(&stack);
            Leftmost_Preorder(helper->RPtr, &stack);
        }
    }
}
...
**/
    ...
    while (!SEmptyP(&stackP)) {
        tBTNodePtr helper = STopPopP(&stackP);
        SPushP(&stackP, helper);
        bool left = STopPopB(&stackB);
        if (left) {
            SPushB(&stackB, FALSE);
            Leftmost_Postorder(helper->RPtr, &stackP, &stackB);
        } else {
            STopPopP(&stackP);
            BTWorkOut(helper);
        }
