...
UPDATE Pirat
SET plavi_se_na = (SELECT id from Lod WHERE patri IN (SELECT id from Pirat WHERE prezdivka = 'Červený šátek'))
...
VALUES ((SELECT id FROM Pirat where prezdivka = 'Červený šátek'),
        (SELECT id FROM Lod where patri in ((SELECT id FROM Pirat where prezdivka = 'Červený šátek'))));
-- doplneni vztahu
INSERT INTO Lod_bojovala_v_bitva (id_lod, id_bitva)
VALUES ((SELECT id FROM Lod where patri in ((SELECT id FROM Pirat where prezdivka = 'Červený šátek'))),
